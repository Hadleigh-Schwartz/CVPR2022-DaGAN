...
def make_animation(source_video, driving_video, generator, kp_detector, relative=True, adapt_movement_scale=True, cpu=False):
    sources = []
    drivings = []
    with torch.no_grad():
        predictions = []
        ...
        outputs = depth_decoder(depth_encoder(source[:, :, 0]))
        depth_source = outputs[("disp", 0)]
        source_kp = torch.cat((source[:, :, 0],depth_source),1)
        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            out = generator(source_frame, kp_source=kp_source, kp_driving=kp_norm,source_depth = depth_source, driving_depth = depth_map)
    ...
    return None, drivings, predictions,depth_gray
...
if __name__ == "__main__":
    ...
    source_image_reader = imageio.get_reader(opt.source_image)
    fps = source_image_reader.get_meta_data()['fps']
    source_video = []
    try:
        for im in source_image_reader:
            source_video.append(im)
    except RuntimeError:
        pass
    source_image_reader.close()
    ...
    sources, drivings, predictions,depth_gray = make_animation(source_video, driving_video, generator, kp_detector, relative=opt.relative, adapt_movement_scale=opt.adapt_scale, cpu=opt.cpu)
